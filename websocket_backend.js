...
binanceWS.candles(symbol, interval, (candle) => {
    ...
    if (candle.isFinal===true){
        t=candle.startTime;
        trend_dicts.push({open:candle.open,high:candle.high,low:candle.low,close:candle.close})
        if (trend_dicts.length >2){
        trends=d_hl_trendsticks_1(trend_dicts)
        if (!isNaN(trends.open) ){


        candles_dicts.push(trends)


        if (candles_dicts.length >1){
            //opened=candles_dicts[candles_dicts-2].c;
            if (new_candles === false){
            new_candle_sticks.push({open:candles_dicts[candles_dicts.length-2].close,high:candle.high,low:candle.low,close:candle.close})
            }else{
              new_candle_sticks.push({open:new_candle_sticks[new_candle_sticks.length -1].close,high:trends.high,low:trends.low,close:trends.close})
            }
        
            if (new_candle_sticks.length >1){
            new_candles=true
              //new_candle_sticks.push({open:new_candle_sticks[new_candle_sticks.length-2].close,high:message.k.h,low:message.k.l,close:message.k.c})
                if (new_candle_sticks[new_candle_sticks.length-2].close > new_candle_sticks[new_candle_sticks.length-2].open){
                    ...
                    if ((new_candle_sticks[new_candle_sticks.length-1].close > new_candle_sticks[new_candle_sticks.length-2].close) || (new_candle_sticks[new_candle_sticks.length-1].close < new_candle_sticks[new_candle_sticks.length-2].open) ) {
                        ...
                        if (final_candles.length > 3){
        
                          Ver=Var_Fun(final_candles,2,1);
                          hdl=hl_di(final_candles)
                        
                          //candleSeries.update({
                          //time:t/1000,
                          //open: Number(new_candle_sticks[new_candle_sticks.length -1].open),
                          //high :Number(new_candle_sticks[new_candle_sticks.length -1].high),
                          //low:Number(new_candle_sticks[new_candle_sticks.length- 1].low),    
                          //close:Number(new_candle_sticks[new_candle_sticks.length -1].close)
                      //});
                      ...
                      ch_c=ch_candle(chart_dict);
                      chart_dict[chart_dict.length-1]["ch"]=ch_c;
                      //lineS.update({
                                    //time:t/1000,
                                    //value:hdl[hdl.length -2]
                                 // });
                        chart_dict[chart_dict.length-1]["hdl1"]=hdl[hdl.length -2];
        
                                 // lineSe.update({
                                 //   time:t/1000,
                                  //  value:hdl[hdl.length -1]
                                 // });
                        chart_dict[chart_dict.length-1]["hdl2"]=hdl[hdl.length -1];
        
                                  //lineSeries.update({
                                   // time:t/1000,
                                  //  value:Ver[Ver.length - 1]
                                  //});
                        chart_dict[chart_dict.length-1]["ver1"]=Ver[Ver.length - 1];
                                  //lin.update({
                                   // time:t/1000,
                                   // value:Ver[Ver.length - 2]
                                  //});}
                        chart_dict[chart_dict.length-1]["ver2"]=Ver[Ver.length - 2];
                                  
                      rp=d_hl_trendsticks(chart_dict);
                      chart_dict[chart_dict.length-1]["trend"]=rp[rp.length - 1];
                      }
                        ...
                        //candleSeries.update({
                        //    time:t/1000,
                        //    open: Number(new_candle_sticks[new_candle_sticks.length -1].close),
                        //    high :Number(new_candle_sticks[new_candle_sticks.length -1].close),
                        //    low:Number(new_candle_sticks[new_candle_sticks.length -1].close),    
                        //    close:Number(new_candle_sticks[new_candle_sticks.length -1].close)
                        //})
            
            
            
                    } }else if (new_candle_sticks[new_candle_sticks.length-2].close < new_candle_sticks[new_candle_sticks.length-2].open){
                        ...
                        if ((new_candle_sticks[new_candle_sticks.length-1].close > new_candle_sticks[new_candle_sticks.length-2].close) || (new_candle_sticks[new_candle_sticks.length-1].close < new_candle_sticks[new_candle_sticks.length-2].close) ) {
         ...
         // }));
          ...
          console.log(trends,trend_dicts,chart_dict)

}

    // Send the raw Kline data to connected clients (frontend)
    wss.clients.forEach((client) => {
        client.send(JSON.stringify(chart_dict));
